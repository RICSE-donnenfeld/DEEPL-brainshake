...
The CNN achieves higher accuracy than the random forest, demonstrating the advantage of end-to-end deep learning for seizure detection.
\end{abstract}
...
\begin{description}
  \item[CNN] Convolutional neural network. Supervised classifier (seizure vs.\ non-seizure).
  \item[CV] Cross-validation. Here: 5-fold patient-level CV (splits by patient, not by window).
  ...
  \item[Balanced accuracy] Mean of sensitivity (recall) and specificity: $(\mathrm{TPR}+\mathrm{TNR})/2$.
...
All experiments use patient-level 5-fold cross-validation (CV): each fold holds out a disjoint set of patients for validation/testing, and all windows belonging to those patients are excluded from training.
This split strategy prevents data leakage across folds because windows from the same subject never appear in both training and evaluation.

To ensure reproducibility, all splits and model training runs are executed with a fixed random seed (2026) and the same CLI configuration used by the project pipeline.
...
Because seizure windows are rare compared to non-seizure windows, accuracy alone can be overly optimistic.
We therefore also report recall (sensitivity) and balanced accuracy to capture seizure detection performance under class imbalance.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\linewidth}
\includegraphics[width=\linewidth]{average_recall.png}
\caption{Average recall across folds}
\label{fig:avg_recall}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
\includegraphics[width=\linewidth]{average_balanced_accuracy.png}
\caption{Average balanced accuracy across folds}
\label{fig:avg_balanced_accuracy}
\end{subfigure}
\caption{Metrics that are robust to severe class imbalance.}
\label{fig:avg_imbalance_metrics}
\end{figure}
...
\noindent \textbf{Summary.} The accuracy plots show stable generalization across patient folds, while recall and balanced accuracy highlight how well each method detects seizures despite the strong label imbalance.
These plots are based on the exported metrics and figures under \texttt{out/}.
